...
NRE_DEFAULT = 14.8e6
N_PRODUCTS  = 200        # fleet size; uniform across paradigms (see nre figure)
...
def total(key, params, nre, price, life, volume):
    p = AREAS[key]
    # NRE split per-design / per-product (see generate_nre_cost_figure.py).
    # 14.0/14.8 of the bundle follows the tapeout count, 0.8/14.8 follows the
    # package count, which is N_PRODUCTS for every paradigm.
    nre_design  = nre * (14.0 / 14.8)
    nre_product = nre * (0.8 / 14.8)
    t = (re_cost(p, params)
         + (p['n_unique_designs'] * nre_design + N_PRODUCTS * nre_product) / volume)
    if INCLUDE_OPCOST:
...
_WAFER0    = gc.effective_wafer_cost(_DP)                   # $/wafer at the selected CATCH layer
_D0        = gc.effective_defect_density(_DP)               # defects / mm^2 at that layer
...
SWEEPS = [
    ...
    ("Wafer cost",         f"\\${_WAFER0*0.5:.0f}",       factor(params=P(wafer_cost_scale=0.5)),
                           f"\\${_WAFER0*2.0:.0f}",       factor(params=P(wafer_cost_scale=2.0))),
